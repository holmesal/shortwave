        ...
        {(error:NSError!, user:FAUser!) in
            ...
            let completion:()->() =
            {
                if let e = error
                {
                    ...
                    var alert: UIAlertView = UIAlertView(title:"No account found.", message: "I was unable to find a Facebook account on this device, are you sure your phone has a Facebook account?  Go to settings then privacy.", delegate: self, cancelButtonTitle: nil, otherButtonTitles:"I'll check")
                    alert.show()
